...
class hdf5_packager(packager):
    ...
    def package_image(self, image, timestamp, img_idx):
        ...
        # image_ts_dset = self.events_file.create_dataset("images_ts/image{:09d}".format(img_idx),
        #         data=timestamp, dtype=np.dtype(np.float32))
        

    def package_image_ts(self, timestamp, img_idx):
        image_ts_dset = self.events_file.create_dataset("images_ts/image{:09d}".format(img_idx),
                data=timestamp, dtype=np.dtype(np.float32))
